            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="bg-orange-200">
                                ...
                                @foreach($premises as $prem)
                                    <div class="bg-gray-200 rounded-lg p-4 ml-4 text-center mb-4">
                                        <div class="h-40 bg-gray-400 rounded-lg">
                                            @if($prem->images !== null)
                                            <img src="{{ asset('storage/' . json_decode($prem->images, true)[0]) }}"
                                                 class="rounded flex" style="width: 350px; height: 200px">
                                            @endif
                                        </div>
                                        <div class="mt-16">
                                            <h4 class="text-xl font-semibold">{{ $prem->title }}</h4>
                                            <p class="text-sm mt-2">{{ $prem->description }}</p>
                                            <p class="text-sm mt-2 text-lg">{{ $prem->city }}</p>
                                            <p class="text-sm mt-2 font-bold">Powierzchnia: {{ $prem->area }}
                                                m<sup>2</sup></p>
                                            <p class="text-sm mt-2 italic">Cena: {{ number_format($prem->price, 0 , ',', ' ') }} PLN</p>

                                            @if(Auth::guest())
                                                <a href="{{ route('premises.show', $prem->id) }}">
                                                    <button
                                            ...
                                            @else
                                                <a href="{{ route('premises.edit', $prem->id) }}">
                                                    <button
                                                ...
                                                <a href="{{ route('premises.delete', $prem->id) }}">
                                                    @csrf
                                                    <button
                        ...
                        <div class="p-6">
                            {{ $premises->appends($next_query)->links() }}
                        </div>
                    </div>
                </div>
            </div>
