                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="bg-orange-200">
                        ...
                        <div class="flex w-screen">
                            <div class="max-w-6xl">
                                @if(sizeof($flatsRent) === 0)
                                    @include('layouts.nothing-found')
                                @endif
                            </div>
                            <div class="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 max-w-6xl">
                                @foreach($flatsRent as $flatRent)
                                    <div class="bg-gray-200 rounded-lg p-4 ml-4 text-center mb-4">
                                        <div class="h-40 bg-gray-400 rounded-lg">
                                            <img src="{{ asset('storage/' . json_decode($flatRent->images, true)[0]) }}"
                                                 class="rounded flex" style="width: 350px; height: 200px">
                                        </div>
                                        <div class="mt-16">
                                            <h4 class="text-xl font-semibold">{{ $flatRent->title }}</h4>
                                            <p class="text-sm mt-2">{{ $flatRent->description }}</p>
                                            <p class="text-sm mt-2 text-lg">{{ $flatRent->city }}</p>
                                            <p class="text-sm mt-2 font-bold">Powierzchnia: {{ $flatRent->area }}
                                                m<sup>2</sup></p>
                                            <p class="text-sm mt-2 italic">
                                                Cena: {{ number_format($flatRent->price, 0 , ',', ' ') }} PLN</p>

                                            @if(Auth::guest())
                                                <a href="{{ route('flats.show', $flatRent->id) }}">
                                                    <button
                                            ...
                                            @else
                                                <a href="{{ route('flats-rent.edit', $flatRent->id) }}">
                                                    <button
                                                ...
                                                <a href="{{ route('flats-rent.delete', $flatRent->id) }}">
                                                    @csrf
                                                    <button
                        ...
                        <div class="p-6">
                            {{ $flatsRent->appends($next_query)->links() }}
                        </div>
                    </div>
                </div>
            </div>
